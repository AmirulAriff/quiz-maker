...
class BFDetScreen(Screen):
    ...
    def on_breakfast(self, *args):
        query = "SELECT bf_ing FROM breakfast ORDER BY bf_id ASC"

        cursor.execute(query)
        querycheck = cursor.fetchall()

        for i in querycheck:
            button = Button(text=str(i[0]), color=(1, 0, 1, 1), background_color=(0, 0, 0, 1),
                            outline_color=(1, 0, 1, 1))
            button.bf_id = fooddetail
            self.bfdet.add_widget(button)
...
class BFAnalysisScreen(Screen):
    ...
    def press(self, button):
        ...
        if weekno == 0:
            ...
            print("thursday")
            query = "insert into thursday (user_id, foo_id, mealtype) values ("+str(userLogId)+", "+str(button.foo_id)+", 'Breakfast')"
        elif weekno == 4:
            print("Friday")
    ...
    def on_box(self, *args):
        ...
        for i in querycheck:
            button = Button(text=str(i[1]), color=(1, 0, 1, 1), background_color=(0, 0, 0, 1), outline_color=(1, 0, 1, 1))
            button.foo_id = i[0]
